...
export async function Classifica({ teamId, leagueId }: { teamId: string; leagueId: string }) {
  const db = supabaseAdmin();

  const [{ data: generale }, { data: perGiornata }] = await Promise.all([
    db.from('v_tipster_classifica').select('*').eq('league_id', leagueId),
    db.from('v_tipster_giornata').select('*').eq('league_id', leagueId),
  ]);
  ...
  return (
    <>
      ...
      <h2 style={{ marginTop: 16 }}>Generale</h2>
      <div className="panel">
        <div className="tablewrap">
          <table>
            ...
            <tbody>
              {classifica.map((r, i) => (
                <tr key={r.team_id} style={r.team_id === teamId ? { background: 'var(--accent-soft)' } : undefined}>
